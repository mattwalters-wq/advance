...
// ============ COMMENTS PANEL ============
function CommentsPanel({ postId, postAuthorId, supabase, currentUserId, currentProfile, onClose, onCommentAdded }) {
  ...
  const commentBody = (c, prof, isBand, displayName, canDelete, canEdit) => (
    <>
      <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 3 }}>
        <span style={{ fontSize: 12, fontWeight: 600, color: INK }}>{displayName?.toLowerCase()}</span>
...
// ============ POST CARD ============
function PostCard({ post, currentUserId, currentProfile, supabase, onRefresh }) {
            ...
            {loadingPosts ? (
              ...
              posts.map((post, i) => (
                <div key={post.id} style={{ animation: `fadeIn 0.35s ease-out ${i * 0.04}s both` }}>
                  <PostCard post={post} currentUserId={user?.id} currentProfile={profile} supabase={supabase} onRefresh={fetchPosts} />
                </div>
              ))
            )}
          </div>
                    ...
                    {shows[region].map((show, i) => {
                      ...
                      return (
                        <div key={show.id} style={{
                          ...
                          opacity: sold && !attended ? 0.4 : 1,
                        }}>
                          <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
                            <div style={{ fontFamily: "'DM Mono', monospace", fontSize: 11, color: SLATE, minWidth: 52 }}>{formatShowDate(show.date)}</div>
                            <div style={{ flex: 1 }} onClick={() => show.ticket_url && !sold && window.open(show.ticket_url, "_blank")} style={{ flex: 1, cursor: show.ticket_url && !sold ? "pointer" : "default" }}>
