      ...
      <div style={{ borderBottom: `1px solid ${border}`, padding: '16px 24px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
          ...
          <div>
            ...
            <div style={{ fontSize: '13px', color: muted, fontStyle: 'italic' }}>{artist?.project}</div>
          </div>
        </div>
          ...
          <>
            <div style={{ marginBottom: 24, display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
              <button onClick={handleShare} disabled={sharing}
                style={{
                  padding: '10px 20px',
                  background: copied ? '#2d7a4f' : accent,
                  color: '#fff', border: 'none', borderRadius: 8,
                  cursor: 'pointer', fontFamily: 'monospace',
                  fontSize: 10, letterSpacing: 2, transition: 'background 0.2s'
                }}>
                {copied ? '✓ LINK COPIED' : sharing ? '...' : '🔗 SHARE CREW LINK'}
              </button>
              {copied && (
                <span style={{ fontSize: 12, color: '#2d7a4f' }}>
                  Send to band & crew — no login needed
                </span>
              )}
              {selectedTour.share_token && !copied && (
                <span style={{ fontSize: 11, color: muted, fontFamily: 'monospace' }}>
                  advance-brown.vercel.app/tour/{selectedTour.share_token}
                </span>
              )}
            </div>
            ...
            <div style={{ display: 'grid', gap: '20px' }}>
              {shows.length > 0 && (
                <div style={{ background: card, borderRadius: '12px', padding: '20px', border: `1px solid ${border}` }}>
                  <div style={{ fontSize: '11px', letterSpacing: '0.1em', color: muted, marginBottom: '16px', textTransform: 'uppercase', fontFamily: 'monospace' }}>Shows — {shows.length}</div>
                  {shows.map((show, i) => (
              ...
              {travel.length > 0 && (
                <div style={{ background: card, borderRadius: '12px', padding: '20px', border: `1px solid ${border}` }}>
                  <div style={{ fontSize: '11px', letterSpacing: '0.1em', color: muted, marginBottom: '16px', textTransform: 'uppercase', fontFamily: 'monospace' }}>Travel — {travel.length} items</div>
                  {travel.map((t, i) => (
              ...
              {accommodation.length > 0 && (
                <div style={{ background: card, borderRadius: '12px', padding: '20px', border: `1px solid ${border}` }}>
                  <div style={{ fontSize: '11px', letterSpacing: '0.1em', color: muted, marginBottom: '16px', textTransform: 'uppercase', fontFamily: 'monospace' }}>Accommodation — {accommodation.length}</div>
              ...
              {contacts.length > 0 && (
                <div style={{ background: card, borderRadius: '12px', padding: '20px', border: `1px solid ${border}` }}>
                  <div style={{ fontSize: '11px', letterSpacing: '0.1em', color: muted, marginBottom: '16px', textTransform: 'uppercase', fontFamily: 'monospace' }}>Contacts — {contacts.length}</div>
                  {contacts.map((c, i) => (
