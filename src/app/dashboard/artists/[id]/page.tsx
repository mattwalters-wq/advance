...
type ModalType = 'show' | 'travel' | 'accommodation' | 'contact' | null
...
export default function ArtistPage() {
  ...
  async function handleSave() {
    ...
    if (modal === 'show') {
      await supabase.from('shows').insert({ ...base, ...form })
    } else if (modal === 'travel') {
      await supabase.from('travel').insert({ ...base, ...form })
    } else if (modal === 'accommodation') {
      await supabase.from('accommodation').insert({ ...base, ...form })
    } else if (modal === 'contact') {
      await supabase.from('contacts').insert({ ...base, ...form })
    }
          ...
          <div style={{ background: card, borderRadius: 16, padding: 28, width: '100%', maxWidth: 480, maxHeight: '90vh', overflowY: 'auto' }}
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 24 }}>
              <div style={{ fontFamily: 'monospace', fontSize: 11, letterSpacing: 3, textTransform: 'uppercase', color: muted }}>
                Add {modal === 'accommodation' ? 'Hotel' : modal}
              </div>
