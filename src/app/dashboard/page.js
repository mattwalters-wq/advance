...
// ============ SHOWS MANAGER ============
const ShowField = ({ label, value, onChange, type = "text", options }) => (
    ...
    <div>
      ...
      <div style={{ background: OFF_WHITE, borderRadius: 3, padding: "18px", border: "1px solid #E8E2D8", marginBottom: 20 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0 12px" }}>
          <ShowField label="date" type="date" value={form.date} onChange={(v) => setForm({ ...form, date: v })} />
          <ShowField label="city" value={form.city} onChange={(v) => setForm({ ...form, city: v })} />
          <ShowField label="venue" value={form.venue} onChange={(v) => setForm({ ...form, venue: v })} />
          <ShowField label="country" value={form.country} onChange={(v) => setForm({ ...form, country: v })} />
          <ShowField label="region" value={form.region} onChange={(v) => setForm({ ...form, region: v })} options={["australia", "europe", "uk", "north_america", "other"]} />
          <ShowField label="status" value={form.status} onChange={(v) => setForm({ ...form, status: v })} options={["announced", "on_sale", "door_sales", "sold_out", "cancelled"]} />
        </div>
        <ShowField label="ticket url" value={form.ticket_url} onChange={(v) => setForm({ ...form, ticket_url: v })} />
        <div style={{ display: "grid", gridTemplateColumns: "1fr auto", gap: "0 12px", alignItems: "end" }}>
          <ShowField label="check-in code (fans enter this at the venue)" value={form.checkin_code} onChange={(v) => setForm({ ...form, checkin_code: v.toUpperCase() })} />
    ...
    <div>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 16 }}>
        <input
          ...
          style={{
            flex: 1, padding: "9px 12px", background: CREAM, border: `1px solid ${DARK_CREAM}`,
            borderRadius: 3, fontSize: 13, color: INK, outline: "none", fontFamily: "'DM Sans', sans-serif",
          }}
        />
        <span style={{ fontFamily: "'DM Mono', monospace", fontSize: 10, color: SLATE, flexShrink: 0 }}>
          {members.filter((m) => {
            if (!search.trim()) return true;
            const q = search.toLowerCase();
            return (m.display_name || "").toLowerCase().includes(q) || (m.email || "").toLowerCase().includes(q) || (m.city || "").toLowerCase().includes(q);
